 ...
 * 1. 값을 담을 propterty 정의
 * 2. 정해진 규칙에 따라 사용할 수 있도록 property 캡슐화
 * 3. 캡슐화 된 프로퍼티에 접근할 수 있는 인터페이스 정의
 * 		getter/setter
 * 		get[set] 프로퍼티명(initical capitalize) ex) getProperty_name
 * 4. 객체의 상태를 비교할 수 있는 방법 제공(equals 재정의)
 * 5. 객체의 상태를 확인할 수 있는 방법 제공 (toString 재정의)
 * 6. 객체의 상태를 직렬화 할 수 있도록.(implements Serializable)
 * * 직렬화/역직렬화 :
 *  직렬화 : 객체를 전송이나 저장을 목적으로 바이트 집합의 형태로 변환하는 작업.
 *  역직렬화 : 바이트 집합의 형태로 전송되거나 저장되어있던 객체를 다시 원래의 형태로 복원하는 작업
 * 
 */
